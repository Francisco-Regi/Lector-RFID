...
void setup() {
  ...
  Serial.println("RC522 listo - V6.1 Fix");
}
...
void readAllMemory(byte* buffer, int& index, int maxLen) {
  ...
  // --- ESTRATEGIA PARA TARJETAS MIFARE CLASSIC ---
  if (type != MFRC522::PICC_TYPE_MIFARE_UL) {
      for (byte sector = 1; sector <= 4; sector++) { 
          byte trailer = sector * 4 + 3;
          
          // Intento 1: Llave de Fabrica
          if (mfrc522.PCD_Authenticate(MFRC522::PICC_CMD_MF_AUTH_KEY_A, trailer, &keyDefault, &(mfrc522.uid)) != MFRC522::STATUS_OK) {
             // Si falló, reintentamos con Llave NDEF
             mfrc522.PICC_HaltA(); 
             mfrc522.PCD_StopCrypto1(); 
             
             // --- CORRECCIÓN DEL ERROR AQUI ---
             // Usamos variables temporales del tipo correcto (byte)
             byte bufferATQA[2]; 
             byte bufferSize = sizeof(bufferATQA);
             mfrc522.PICC_WakeupA(bufferATQA, &bufferSize); 
             // ---------------------------------

             if (mfrc522.PCD_Authenticate(MFRC522::PICC_CMD_MF_AUTH_KEY_A, trailer, &keyNDEF, &(mfrc522.uid)) != MFRC522::STATUS_OK) {
                continue; // Si ambas fallan, saltamos el sector
             }
          }

          // Leemos los 3 bloques
          for (byte i = 0; i < 3; i++) {
